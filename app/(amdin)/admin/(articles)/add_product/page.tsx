...
const AddProduct = () => {
        ...
        onSubmit: async (values) => {
            ...
            if (values.category != "" && characteristicValues.length != 0) {
                if (ImagesArray.length != 0) {
                    const articleModel = new ArticleModel(values.name, Number(values.price), values.description, ImagesArray, Number(values.category), Number(values.tax));
    ...
    return (
        <main className={'my-20 px-20 flex flex-col space-y-10'}>
            <HeadDetaill title={"Ajouter un Arcticle"} subtitle={"Gestion des articles"}/>
                ...
                <form onSubmit={formik.handleSubmit} className={'flex flex-col space-y-10 w-[950px]'}>
                    ...
                    <div>
                        ...
                        <div className={"grid grid-cols-1 md:grid-cols-4 gap-4"}>
                            <div>
                                <div
                                    className={`w-full h-[200px] overflow-hidden relative items-center justify-center content-center rounded-2xl border-2`}>
