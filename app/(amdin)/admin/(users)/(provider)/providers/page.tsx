...
export default function Providers() {
    ...
    useEffect(() => {
        Api.read("/api/provider/getProviderByStatus/PROVIDER").then((values) => {
            setData(values);
        })
    }, []);
    ...
    return (
        <div className={'mt-20 px-20 flex flex-col space-y-10'}>
            <HeadList title={"Fournisseurs"} subtitle={"Gestion des utilisateurs"} link={"/admin/add_provider"}
                      buttonTitle={"Ajouter une fournisseur"} count={data.length}/>

            <Separator className={'w-full my-5'}/>

            {/*search input*/}
            {/*search input*/}
            <SearchInput query={query} setQuery={setQuery} data={data} setResults={setResults}
                         palceholder={"Recherche par nom"} element={"name"}/>
